...
function playSfx(type) {
    ...
    if (type === 'click') {
        osc.frequency.setValueAtTime(600, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.1);
        osc.start(now); osc.stop(now + 0.1); if(navigator.vibrate) navigator.vibrate(5);
    } else if (type === 'swoosh') {
        osc.type = 'triangle'; gain.gain.setValueAtTime(0.05, now); gain.gain.linearRampToValueAtTime(0, now + 0.15);
        osc.start(now); osc.stop(now + 0.15);
    } else if (type === 'success') { 
        [440, 554, 659].forEach((f, i) => { const o = audioCtx.createOscillator(); const g = audioCtx.createGain(); o.connect(g); g.connect(audioCtx.destination); o.frequency.value = f; g.gain.exponentialRampToValueAtTime(0.001, now + 0.5 + (i*0.1)); o.start(now); o.stop(now + 0.5); });
    }
}
...
function triggerAdminUnlock() {
    adminTapCount++;
    if (adminTapCount === 5) {
        if(prompt("🔐 GOD MODE ACCESS PIN:") === "2025") { 
            alert("Bienvenido, CEO."); switchTab('admin'); loadAdminStats(); fetchAdminModeration();
...
async function adminCreateClash() {
    ...
    const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1); const dateStr = tomorrow.toISOString().split('T')[0];
    await db.from('clashes').delete().eq('publish_date', dateStr);
    await db.from('clashes').insert({ option_a: a, option_b: b, publish_date: dateStr, votes_a: 0, votes_b: 0 });
    playSfx('success'); alert(`✅ Dilema programado para mañana (${dateStr}).`);
    document.getElementById('admin-opt-a').value = ""; document.getElementById('admin-opt-b').value = "";
}
...
async function fetchAdminModeration() {
    ...
    } else { document.getElementById('admin-sug-text').innerText = "No hay nada pendiente."; adminJudgeId = null; }
}
...
async function shareScreenshot(type) {
    ...
    if (type === 'oracle') { textDiv.innerHTML = `"${document.getElementById('q-text').innerText}"`; }
    else if (type === 'clash') {
        const winText = clashData.va > clashData.vb ? clashData.a : clashData.b;
        const total = clashData.va + clashData.vb; const perc = total===0 ? 0 : Math.round((Math.max(clashData.va, clashData.vb)/total)*100);
    ...
    } catch (err) { console.error(err); alert("Error imagen."); }
}

// CORE & IDENTITY
async function initUser() {
    if (!currentUser.id) {
        const { data } = await db.from('profiles').insert([{ username: currentUser.name, avatar: currentUser.avatar, streak: 1, last_visit: new Date().toISOString() }]).select().single();
...
function checkStreakCloud(cloudData) {
    ...
    if (lastVisit !== today) {
        ...
        if (lastVisit === yesterday.toISOString().split('T')[0]) { currentUser.streak++; setTimeout(() => playSfx('success'), 800); } 
...
function updateProfileUI() {
    if(!document.getElementById('profile-name')) return; // Safety check
...
function switchTab(t, el) {
    ...
    ['oracle', 'clash', 'judgment', 'profile', 'admin'].forEach(s => {
